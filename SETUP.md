...
| Meta error 139002 "Flow JSON validation" (Deploy Flow) | Invalid Flow JSON structure | Flow was created as draft but publish failed. Use a **supported** Flow JSON version (see changelog: **5.1**, **6.0–6.3**, **7.0–7.3**; **7.3** recommended). Use **data_source** as a direct array of `{ "id", "title" }` for static dropdowns; use **data: {}** on screens (no `__example__`); avoid invalid characters in titles. Re-run Deploy Flow after fixing `flows/farmer-registration.json`. |
