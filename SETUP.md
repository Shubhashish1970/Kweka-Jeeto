...
| Meta error 139002 "Flow JSON validation" (Deploy Flow) | Invalid Flow JSON structure | The deploy script now creates the flow as **draft** first and prints **validation_errors** (with paths and messages) if any; fix those in `flows/farmer-registration.json` and re-run. Per [Meta Error Codes](https://developers.facebook.com/docs/whatsapp/flows/reference/error-codes/): (1) **Navigate payload**: the next screen must have a **data** model declaring every field you pass in the navigate payload (e.g. FARMER_DETAILS must have `data: { entry_point: { type: "string" }, ... }` if WELCOME navigates with `payload: { entry_point: "welcome" }`; CROP_SELECTION must have `data: { farmer_name, age, ... }` matching FARMER_DETAILS navigate payload). (2) Use **7.3**; Dropdown uses **data-source** (hyphen); flow name only `[a-z0-9_]`. |
| Meta "versions in flow JSON file are not available for publishing" (Deploy Flow) | Flow JSON version is **frozen** or unsupported | **5.0** is frozen (Sept 2025). Use a version that is currently allowed for **publishing**: **5.1**, **6.0**, **6.1**, **6.2**, **6.3**, **7.0**, **7.1**, **7.2**, or **7.3** (recommended). See [Meta Flows Changelog](https://developers.facebook.com/docs/whatsapp/flows/changelogs). Set `"version": "7.3"` in `flows/farmer-registration.json` and re-run Deploy Flow. |
...
| **Error 131009: "Specified screen WELCOME is not allowed as first screen... Allowed screen name is: FARMER_DETAILS"** | The published flow had no explicit entry screen, so Meta defaulted to FARMER_DETAILS. | The Flow JSON now includes a **routing_model** with WELCOME → FARMER_DETAILS → CROP_SELECTION so WELCOME is the entry screen. **Re-run Deploy Flow (One-Time)** to publish this flow, then **update the FLOW_ID** GitHub Secret (and Admin Config) with the new value from the workflow output. The backend sends **screen: WELCOME** so the flow opens on the welcome screen. |
| **Welcome screen or images do not show in the flow** | The flow running on WhatsApp is an older version without the welcome screen or Meta could not load images. | (1) **Re-deploy the flow:** Any change to `flows/farmer-registration.json` (e.g. adding the welcome screen or image URLs) requires re-running **Actions → Deploy Flow (One-Time)**. Copy the new **FLOW_ID** from the workflow output and update the **FLOW_ID** GitHub Secret (and Admin Config if you use it). (2) **Images:** Ensure image URLs in the flow JSON are **HTTPS** and publicly accessible. Images are placed at the **top** of Farmer Details and Crop Selection so they appear without scrolling. (3) **Flow invite (chat bubble) image:** To show an image in the Register message bubble, set config key **flow_header_image** (Admin or API) to a public image URL (e.g. your welcome hero image). If the API rejects image headers for flows, the backend falls back to text header. |
