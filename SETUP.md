...
| Meta error 139002 "Flow JSON validation" (Deploy Flow) | Invalid Flow JSON structure | Flow was created as draft but publish failed. Per [Meta Error Codes](https://developers.facebook.com/docs/whatsapp/flows/reference/error-codes/): (1) **Navigate payload**: the next screen must have a **data** model declaring every field you pass in the navigate payload (e.g. CROP_SELECTION must have `data: { farmer_name: { type: "string" }, ... }` if FARMER_DETAILS navigates with those fields). (2) Use supported version **7.3**; Dropdown must use **data-source** (hyphen), not `data_source`; value is array of `{ "id", "title" }`; flow name only `[a-z0-9_]`. Re-run Deploy Flow after fixing `flows/farmer-registration.json`. |
